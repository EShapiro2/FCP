...
expand(Command, Xs, Ys) :-
  ...
  Command = "_complete_names"(Head, Tail) :
    Xs = Ys,
    Names = [abort, compile, debug, date, info, lint, path,
	     resolvent, resume, rpc, profile, suspend, time,
	     add, computation, service, signal,
	     close, events, edit, extract, input, load, services,
	     diagnostic, silent, quiet, verbose, start, state | Head], 
    Tail =  [reset, status, cdg, ndg, nccdg, type_check, wam,
	     license, warranty] |
	computation # sio(complete_names(Names));

  Command = "_all_functors"(Head, Tail) :
    Tail =  [abort, compile, debug, date, info, lint, path,
	     resolvent, resume, rpc, profile, suspend, time,
	     add, computation, service, signal,
	     close, events, edit, extract, input, load, services,
	     diagnostic, silent, quiet, verbose, start, state,
	     cd, reset, status, cdg, ndg, nccdg, type_check, wam,
	     "!", "=", "^", ":=", ","],
    Xs = Ys |
	sort_services(Head, service, Sorted),
	make_table # rectangle(Sorted, [], Table),
	computation # display(stream, Table?, [type(ground)]);
...
add_to_stream(X,Ms) :-
  ...
  otherwise |
        computation # display(term, "can't_append_to_stream"(Ms!X)).
...
close_stream(Ms) :-
   ...
   otherwise |
        computation # display(term, "can't_close_stream"(Ms!)).
...
less(Doc) :-
    ...
    Slash := ascii('/'),
    Ch =:=  Slash |
	utils#append_strings(["less ", Doc], Less),
	to_unix(Less?);
    ...
    otherwise :
      Root = [] |
	Root#"_unique_id"(UID),
	utils#append_strings(["less ", UID, Doc], Less),
	to_unix(Less?);

    otherwise |
	computation#display("not a string"(Doc)).

to_unix(Command) :-
    known(Command) |
	processor # interface(unix_call(Command)).
