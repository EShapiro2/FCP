...
run(Commands) :-
	run(Commands, _System, _Events, _Out).
run(Commands, System) :-
	run(Commands, System, _Events, _Out).

run(Commands, System, AmbientEvents) + (Out = _) :-
	computation # spi_monitor # scheduler(Scheduler),
	synchronize.

synchronize(Commands, System, AmbientEvents, Out, Scheduler) :-
    vector(Scheduler) :
      make_vector(STATUS_VECTOR_SIZE, Status, Streams),
      Streams = _,
      store_vector(CURRENT_STATUS, running, Status, Status'),
      store_vector(FINAL_STATUS, _Ended, Status', Status''),
      SharedChannels = [],
      InTuple = {In, _AmbientId},
      make_channel(Debug, Out),
      Children = [Ambient?],
      /* Informs controlling service of start of ambient */
      make_vector(AMBIENT_ARITY, System, InTuple),
      store_vector(AMBIENT_ID, system, System) |
	spi_status # get_status(cutoff_status, Cutoff),
	look_ahead,
/*
screen#display(Streams,type(namevars)),
screen#display((finally:cutoff = Cutoff), type(ground)),

Streams = {Current, Final},
screen#display_stream(Cutoff,
			[known(Cutoff),type(namevars), prefix(cutoff)]),
screen#display_stream(Current,
			[known(Current),type(namevars), prefix(current)]),
screen#display_stream(Final,
			[known(Final),type(namevars), prefix(final)]),
*/
	computation # events(Events),
	computation # self # service_id(SId),
	serve_system + (Ambient = System, UniqueId = 0),
	ambient + (AmbientName = public, Parent = System).

  look_ahead(In, Cutoff, Status) :-

    In ? abort,
    read_vector(FINAL_STATUS, Status, Final) :
      In' = _,
      Cutoff = _,
      Final = aborted,
      store_vector(CURRENT_STATUS, Final, Status);
    ...
    Final =\= aborted :
      In' = _,
      Cutoff = _,
      store_vector(FINAL_STATUS, aborted, Status),
      store_vector(CURRENT_STATUS, aborted, Status);

    In ? suspend(_Ready),
    read_vector(CURRENT_STATUS, Status, CurrentStatus),
    CurrentStatus =?= running :
      store_vector(CURRENT_STATUS, suspended, Status) |
	self;

    In ? resume(_Ready),
    read_vector(CURRENT_STATUS, Status, CurrentStatus),
    CurrentStatus =?= suspended :
      store_vector(CURRENT_STATUS, running, Status) |
	self;
    ...
    read_vector(FINAL_STATUS, Status, Final) :
      Cutoff' = _,
      Final = done,			/* beware race with abort */
      store_vector(CURRENT_STATUS, Final, Status),
      store_vector(FINAL_STATUS, Final, Status) |
	self.
...
**   Status(CURRENT_STATUS) - one of running,suspended,done,aborted
**   Status(FINAL_STATUS) - one of done,aborted
...
serve_system(In, Events, Children, UniqueId, Status, SharedChannels,
			Scheduler, Ambient, AmbientEvents, Debug) :-
...
/**************************** External Signals ******************************/
    ...
    read_vector(CURRENT_STATUS, Status, CurrentStatus),
    CurrentStatus =\= running :
      Reply = false(CurrentStatus) |
	self;

    In ? resume(Reply),
    read_vector(CURRENT_STATUS, Status, CurrentStatus) :
    CurrentStatus =?= suspended,
      store_vector(CURRENT_STATUS, CurrentStatus', Status, Status'),
      Reply = CurrentStatus'? |
	DEBUG(resume, CurrentStatus'),
	system_resume(Scheduler, Children, Children', CurrentStatus', Reply),
	self;
...
/****************************** Ambient Tree ********************************/
    ...
    CurrentStatus =?= aborted :
      Tree = resolvent(system, [false(CurrentStatus)], []) |
	self;

    In ? tree(resolvent, Tree),
    read_vector(CURRENT_STATUS, Status, CurrentStatus),
    CurrentStatus =\= running, CurrentStatus =\= aborted :
...
/**************************************************************************/
	...
	close_ambient_events,
	close_debug;

    Events ? aborted,
    read_vector(FINAL_STATUS, Status, FinalStatus) :
      AmbientEvents ! aborted,
      store_vector(CURRENT_STATUS, aborted, Status),
      store_vector(FINAL_STATUS, aborted, Status) |
	DEBUG((system: event-aborted), ambient),
	unify_without_failure(FinalStatus, aborted),
  ...
  close_ambient_events(Status, AmbientEvents) :-
    Status =?= aborted :
      AmbientEvents = [];
    Status =\= aborted :
      AmbientEvents = [terminated].

  system_lookup(In, Events, Children, UniqueId, Status, SharedChannels,
		Scheduler, Ambient, AmbientEvents, Debug, SharedChannel) :-
    known(SharedChannel) |
	serve_system.
  ...
  system_suspend(In, Events, Children, UniqueId, Status, SharedChannels,
	     Scheduler, Ambient, AmbientEvents, Debug, Done, CurrentStatus) :-

    known(Done) :
      store_vector(CURRENT_STATUS, CurrentStatus?, Status, Status') |
	send_to_scheduler(record_data(reset("")), Scheduler, Scheduler'),
	control_children(suspend, Children, Children', Ready),
	system_suspended(Ready?, SharedChannels, CurrentStatus),
  ...
  system_suspended(Ready, SharedChannels, CurrentStatus) :-
	...
	Reply?(CurrentStatus) = true(suspended).

  system_resume(Scheduler, Children, NewChildren, CurrentStatus, Reply) :-
...
**/
ambient(Status, AmbientName, SId, Commands, Parent, Ambient, Debug) :-
    vector(Parent) :
	...
	computation_server#computation([identifier(AmbientId?, _) | Requests],
					...
					ToDomain?,
					Events),
	request_commands(Commands, Done, Requests, Requests'),
...
request_commands(Commands, Done, Requests, EndRequests) :-

    Commands ? Command |
	request_commands(Command, Done, Requests, Requests'?),
	self;
...
*/
    ...
    RelayEvents ? suspended :
      Reply = _,
      write_vector(AMBIENT_CONTROL, suspend(Reply), Ambient) |
	DEBUG(relay-suspend, suspend(Reply)),
	self;


    RelayEvents ? resumed :
      Reply = _,
      write_vector(AMBIENT_CONTROL, resume(Reply), Ambient) |
	DEBUG(relay-resumed, resume(Reply)),
	self;

    RelayEvents ? Other,
    Other =\= aborted,  Other =\= suspended,  Other =\= resumed |
...
serve_ambient(In, Events, FromSub, Done,
	      Ambient, Parent, Status, Children,
...
/* Procedure Services */
	...
	ambient(Status, Name, SId, ModuleName # NewGoal?,
...
/* Capability Initialization */
    ...
    read_vector(AMBIENT_ID, MergingAmbient, FromId) :
      write_channel(record_data(reset(FromId)), Scheduler),
      write_vector(AMBIENT_CONTROL,
...
/* Capability Services */
    ...
    read_vector(AMBIENT_ID, Ambient, AmbientId) :
      write_channel(record_data(reset(AmbientId)), Scheduler),
...
/***************************************************************************/
  ...
  ambient_lookup(In, Events, FromSub, Done,
	...
	spi_utils # make_channel(Channel),
	serve_ambient.
  ...
  record_ambient_item(Done, Really, Wait, Scheduler) :-
    ...
    read_vector(AMBIENT_ID, A2, Id2) :
      Done = _,
      write_channel(record_data(ambient(Action(Id1, Id2))), Scheduler);
...
lookup(Id, PrivateChannel, SharedChannel, AddRefs, ChannelList, NewChannelList,
	 Scheduler, Ambient, Debug, Status) + (Last = "") :-
  ...
  update_new_channel(NewChannel, AddRefs, Type, Rate, WeightTuple,
	...
	unify_without_failure(SharedChannel, NewChannel).
...
merge_local_channels(Idle, Argument, NewArgument, Action,
  ...
  merge_channels(FrozenAtoms, MeltedAtoms, NewArgument, Action,
    ...
    ** not by the receiver of the channel tuple.
    ...
    FrozenAtoms ? channel(Channel),
    vector(Channel) :
      FrozenAtoms'' = [Channel | FrozenAtoms'] |
	self;
...
server_failed(Server, Failure) :-
    ...
    Failure =?= lookup(self(_), PrivateChannel, SharedChannel),
    vector(PrivateChannel),
    vector(SharedChannel) :
      Server = _;

    Failure =?= lookup(self(_), PrivateChannel, SharedChannel, Increment),
    vector(PrivateChannel),
    vector(SharedChannel),
    number(Increment) :
      Server = _;

    Failure =?= lookup(_Locus, PrivateChannel, SharedChannel, Increment),
    vector(PrivateChannel),
    vector(SharedChannel),
    number(Increment),
    read_vector(SPI_CHANNEL_NAME, PrivateChannel, PrivateName),
    read_vector(SPI_CHANNEL_NAME, SharedChannel, SharedName),
    PrivateName =?= SharedName :
      Server = _;
	...
	spi_utils # show_goal(Failure, [], Failure'),
...
format_typerate(Type, Rate, TypeRate) :-
    ...
    Type =:= SPI_DELAY + SPI_RANDOM_FLAG :
      TypeRate = "delay'"(Rate);
