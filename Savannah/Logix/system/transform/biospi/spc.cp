...
/*
       Precompiler for Biological Stochastic Pi Calculus procedures -
       Output Phase.
       February 1999.
*/

-export(stochasticize/4).
-mode(interpret).
...
stochasticize(Language, BlockPrefix, In, Terms) :-
...
output(Language, BlockPrefix, In, ProcessTable, SignatureTable, Terms) :-
	...
	update_publics(RHSS, RHSS', Language, ProcessTable, ProcessTable'?),
  ...
  generate_argument_checks(Atom, RHSS, NewRHSS) :-

    Index := arity(Atom),
    arg(1, Atom, Name) :
      CallId = Name/Index |
	generate_library_calls;
  ...
  generate_library_calls(Index, Atom, CallId, RHSS, NewRHSS) :-

    Index-- > 1,
    arg(Index, Atom, `Argument),
    nth_char(1, Argument, C),
    CHAR_a =< C, C =< CHAR_z :
      RHSS' = (is_channel(CallId, Argument, `Argument), RHSS) |
	self;

    Index-- > 1,
    arg(Index, Atom, `Argument),
    otherwise :
      RHSS' = (isnt_channel(CallId, Argument, `Argument), RHSS) |
	self;
...
stochastic(Language, BlockPrefix, In, ProcessTable, SignatureTable, Terms,
    ...
    RHSS =?= [(Ask : Requests | Body)] :
      Start =
	[write_channel(start(Signature, Operations?, VAR_MESSAGE, VAR_CHOSEN),
		       VAR_SCHEDULER)],
      NewRHSS = (Ask'? : Tell'? | Body),
      Terms ! (AtomPlus? :- Clauses?) |
	...
	/* May need to insert inter-ambient-channels before VAR_SCHEDULER. */
	utils#tuple_to_dlist(Atom, ArgList, InterChannels?),
	utils#list_to_tuple(ArgList, AtomPlus),
	...
	generate_inter_comm(Signature, InterChannels?,
		Communicate2?, Communicate, Terms', Terms''),
	augment_communication,

	utilities#make_predicate_list(';', AugmentedList?, Clauses),
	output.

  communication_to_operations(Communications, Operations, Otherwise) :-
  ...
  augment_communication(ArgList, Otherwise, Communicate, AugmentedList) :-

    Otherwise = true,
    ArgList ? Name :
      Clause = [(otherwise :
		   UnusedArgs? |
		     computation#event(Name - 
			communication_mismatch(VAR_CHOSEN, VAR_MESSAGE)))] |
	extract_unused_arguments(ArgList', UnusedArgs),
	utilities#append(Communicate, Clause, AugmentedList);

    Otherwise =?= false :
      ArgList = _,
      Communicate = AugmentedList.

  extract_unused_arguments(ArgList, UnusedArgs) + (Channels, CC = Channels) :-
    ...
    String =\= MESSAGE,
    String =\= SCHEDULER :
      UnusedArgs = (`String = `"_", UnusedArgs') |
	self;
    ...
    ArgList = [] :
      CC = [] |
	close_unused_channels.

  close_unused_channels(Channels, UnusedArgs) :-

    list(Channels) :
      UnusedArgs = write_channel(close(Tuple?), VAR_SCHEDULER) |
	utils#list_to_tuple(Channels, Tuple);
...
**
**     write_channel(lookup(Locus, Channel, `InterChannelId), VAR_SCHEDULER)
**
...
extract_inter_channels(Lookups, Tail, Tell, InterChannels) :-
    ...
    Lookup =?= _lookup(_Locus, _Channel, InterChannel) :
      Tell ! write_channel(Lookup, VAR_SCHEDULER),
...
generate_inter_comm(Signature, InterChannels,
  ...
  receive_capability(Signature, Kind, Tag, Communicate1, Communicate2,
    ...
    Ask =?= (Select = Tag, Message = []) :
      Ask' = (Select = Tag, Message = {VAR_AMBIENT, VAR_READY}) |
  ...
  send_capability(Signature, Tag, Communicate1, Communicate2,
    ...
    Ask =?= (_Select = Tag),
    Tell =?= (Message = []) :
      Tell' = (Message = {VAR_SCHEDULER, VAR_READY}) |
	complete_capability + (Kind = delegate);

    Communicate1 ? (Ask : Tell | Body),
    Ask =?= (_Select = Tag),
    Tell =?= (Message = [], MoreTell) :
      Tell' = (Message = {VAR_SCHEDULER, VAR_READY}, MoreTell) |
	complete_capability + (Kind = delegate);
  ...
  complete_capability(Signature, Kind, Tag, Communicate1, Communicate2,
    ...
    list_to_string(FL, Complete) :
      ...
      Terms1 = [(Completer? :- known(VAR_READY) | NewBody?) | Terms2] |
	...
	utilities#concatenate_lists([Tell'?,
		[write_channel(Kind(VAR_AMBIENT, VAR_READY), VAR_SCHEDULER)]],
				    Tell''),
	utilities#make_predicate_list(',', Tell''?, Tell'''),
	closed_channels(Tell'?, Closed),
	updated_body,
  ...
  closed_channels(Tell, Closed) :-
    ...
    Tell ? write_channel(close(Channels), VAR_SCHEDULER) :
      Tell' = _ |
...
update_publics(RHSS, NewRHSS, Language, ProcessTable, NextProcessTable) :-

    Language = spifcp,
    RHSS =?=  (computation # public_channels(Publics, Scheduler), Body) :
      NewRHSS = (spi_monitor # public_channels(NewPublics?, Scheduler),
		 Body),
...
extract_spi_channels(Variables, Channels) :-
    ...
    Variables ? channel(`Name), string(Name) :
      Channels ! Name |
	self;
    ...
    Variables ? variable(`Name), string(Name) :
      Channels ! Name |
	self;
