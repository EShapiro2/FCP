...
**
     ...
     Copyright 2007 Ehud Shapiro, Michael Hirsch, William Silverman, Eli Biham
...
terms_to_tokens(Terms, Tokens, Left, Right, Special) :-
    ...
    convert_to_string(Integer, ValueString) :
      Tokens ! ValueString |
	terms_to_tokens;

    Terms ? Real, real(Real) |
	c_output#sprint("G"(0,12,Real),ValueString),
	atomic_term(ValueString, {Tokens, Tokens'}, _, {Right', Right}),
	terms_to_tokens;
...
term_to_tokens(Control, Term, DTs, Context, Priority, Special) :-
    ...
    real(Term) : Context =_, Special = _ |
	c_output#sprint("G"(0,12,Term), S),
	atomic_term(S, DTs, Priority, Control);
