...
-export([get_public_channels/1, get_public_values/1,
	 ...
	 debug/1, end_debug/0, record/1, end_record/1,
	 options/2,
	 reset/0, reset/4, scheduler/1,
	 public_object/3,
	 randomize/0, serialize/0,
	 spi_functions/1, status/1
	]).
...
start_scheduling(Scheduler, Schedule, MathOffset, Ordinal, SpiOffsets,
...
scheduling(Schedule, MathOffset, Ordinal, SpiOffsets, Waiter,
    ...
    read_vector(OBJECT_VALUES, SpiTime, Now) :
      ...
      Waiter = [],
      Record = [],
      Debug = [closed_monitor] |
	unify_without_failure(Cutoff, {_, done(Now)});
    ...
    Schedule ? cutoff(NewLimit, NextState), NewLimit >= 0 :
      RecordLimit = limit(NewLimit),
      RECORD(RecordLimit),
      DEBUG(RecordLimit),
      NextState = State?,
      Cutoff' = {NewLimit, State} |
	unify_without_failure(Cutoff, {_OldLimit, [RecordLimit | State?]}),
    ...
    Schedule ? new_channel(ChannelName, NewChannel, BaseRate) |
	index_channel_name(ChannelName, Ordinal, ChannelName', Ordinal'),
	new_channel + (ComputeWeight = DefaultWeighter),
	continue_waiting;

    Schedule ? new_channel(ChannelName, NewChannel, ComputeWeight, BaseRate),
    ...
    Schedule ? new_public_channel(ChannelName, NewChannel, BaseRate) |
	new_channel + (ComputeWeight = DefaultWeighter),
	continue_waiting;

    Schedule ? new_public_channel(ChannelName, NewChannel,
				  ComputeWeight, BaseRate),
    string(ComputeWeight) |
	new_channel + (ComputeWeight = ComputeWeight(_)),
	continue_waiting;
...
/**************************** Debugger aids *********************************/
    ...
    unknown(Wakeup),
    read_vector(OBJECT_VALUES, SpiTime, Now) :
      RECORD(pausing(Now, Continue?)) |
	pause_scheduling(Unpause, Continue,
			 Schedule', Schedule'',
			 Waiting, Waiting',
			 Wakeup, Wakeup'),
	self;
 ...
 */
    ...
    read_vector(SPI_CHANNEL_NAME, CH2, CNId2) :
      ...
      Waiting' = true,
      Record = [Now,
		end(PrefixedName1(RCId1, SENT_ARROW, CNId1)),
		end(PrefixedName2(RCId2, RECEIVED_ARROW, CNId2)) | Record'?],
      DEBUG(done(Now, PId1(RCId1, CH1), PId2(RCId2, CH2))) |
	self;

    Wakeup =?= true(PId1, RCId1, CH1),
    PId1 =\= (_ : _),
    arg(1, PId1, PName1),
    read_vector(OBJECT_VALUES, SpiTime, Now),
    read_vector(SPI_CHANNEL_NAME, CH1, CNId1) :
      ...
      Waiter ! machine(idle_wait(Wakeup'), _Ok),
      Waiting' = true,
      Record = [Now, end(PName1(RCId1, SENT_ARROW, CNId1)) | Record'?],
      DEBUG(done(Now, PId1(RCId1, CH1))) |
	self;
    ...
    read_vector(SPI_CHANNEL_NAME, CH1, CNId1) :
      ...
      Waiter ! machine(idle_wait(Wakeup'), _Ok),
      Waiting' = true,
      Record = [Now, end(PrefixedName1(RCId1, SENT_ARROW, CNId1)) | Record'?],
      DEBUG(done(Now, PId1(RCId1, CH1))) |
	self;
    ...
    read_vector(OBJECT_VALUES, SpiTime, Now) :
      ...
      RECORD(Idle),
      DEBUG(Idle),
      CutoffStatus = [Idle | CutoffStatus'?],
      Cutoff' = {CutoffLimit, CutoffStatus'} |
	self;
...
pause_scheduling(Unpause, Continue,
  ...
  pause_continue(Unpause, Continue,
		 Schedule, ResetSchedule,
		 Waiting, ResetWaiting,
		 Wakeup, ResetWakeup,
		 SavedInput, SaveInput) :-
...
new_channel(ChannelName, NewChannel, BaseRate, ComputeWeight, Randomize, Reply,
    ...
    otherwise :
      ...
      Scheduler = _,
      SpiOffsets = _,
      Reply = error(new_channel(ChannelName, NewChannel, 
				BaseRate, ComputeWeight)).

DEBUGNOTE(Note) => write_channel(debug_note(Note), Scheduler).
%DEBUGNOTE(Note) => (Scheduler = _, Note = _).
  
  based_or_instantaneous(ChannelName, BaseRate, Result, Scheduler, Reply,
		      Channel, BasedAnchor, InstantaneousAnchor, NewChannel) :-
    ...
    BaseRate =?= infinite :
      ...
      Reply = Result,
      DEBUGNOTE((ChannelName: instantaneous)) |
	queue_channel(Channel, InstantaneousAnchor, NewChannel);
    ...
    RealRate =< 0.0 :
      ...
      NewChannel = Channel,
      Reply = true,
      DEBUGNOTE((ChannelName: sink(unnatural(BaseRate))));
    ...
    RealRate > 0.0 :
      ...
      Reply = Result,
      DEBUGNOTE((ChannelName: based_channel)) |
	queue_channel(Channel, BasedAnchor, NewChannel);

    Result =?= true,
    BaseRate =?= delay(Interval),
    convert_to_real(Interval, RealRate),
    RealRate > 0.0 :
      InstantaneousAnchor = _,
      store_vector(SPI_CHANNEL_RATE, RealRate, Channel),
      store_vector(SPI_CHANNEL_TYPE, SPI_DELAY, Channel),
      Reply = Result,
      DEBUGNOTE((ChannelName: delay_channel)) |
	queue_channel(Channel, BasedAnchor, NewChannel);
    ...
    otherwise :
      ...
      DEBUGNOTE((ChannelName: sink(invalid(BaseRate))));
    ...
    Result =\= true :
      ...
      store_vector(SPI_CHANNEL_TYPE, SPI_SINK, Channel),
      Reply = Result,
      DEBUGNOTE((ChannelName : sink(Result))).
...
queue_channel(Channel, Anchor, NewChannel) :-
...
post_pass0(OpList, Instantaneous, Number, Count, Ok) :-
    ...
    arg(SPI_MS_CHANNEL, Operation, Channel),
    read_vector(SPI_CHANNEL_TYPE, Channel, ChannelType),
    bitwise_and(ChannelType, SPI_TYPE_MASK, SPI_DELAY) |
	self;
...
sum_weights(Channel, RateOffset, Sum, Total) :-
    ...
    vector(Channel'),
    read_vector(SPI_CHANNEL_TYPE, Channel', Type),
    bitwise_and(Type, SPI_TYPE_MASK, SPI_DELAY),
    read_vector(SPI_CHANNEL_RATE, Channel', Rate),
    read_vector(SPI_RECEIVE_WEIGHT, Channel', ReceiveWeight),
    Sum += Rate*ReceiveWeight |
	self;    
...
do_delay_transmit(Channel, Wakeup) :-

    read_vector(SPI_RECEIVE_ANCHOR, Channel, ReceiveAnchor),
    arg(SPI_MESSAGE_LINKS, ReceiveAnchor, ReceiveLinks),
    read_vector(SPI_NEXT_MS, ReceiveLinks, Receive) |
	do_delay_complete(Wakeup, Receive).
