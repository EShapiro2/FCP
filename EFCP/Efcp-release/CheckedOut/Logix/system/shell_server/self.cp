...
serve(Requests, GroundGoals, Forward, Controls) :-
	...
	goals_merger(CompiledGoals, Requests, Commands, Halt),
	user_macros([change_context(SuperId) | Commands], [], Commands1),
	system_macros_merger(Commands1, Commands2, Halt),
...
/************ Protect against filter_user_macros ************/
...
/***********************************************************/
  ...
  Xs ? change_context(NewId) :
     ...
     UMs' ! expand("_complete_names"(Head?, Head), Ys'\Ys'') |
  ...
  Xs ? help :
    Xs'' = ["_all_functors"(Names, Names) | Xs'] |
	user_macros;
...
system_macros_merger(Xs, Ys, Halt) :-
  true : 
    make_channel(DC, Ds) |
	goals(Xs?, Ms?, Ds?, Halt, Xs', DC),
	system_macros.

/*
** serve {_} and filter_system_macro(_,_) commands
** forward others via system_macros service
*/

system_macros(Xs, Ms, Ys, Halt, DC) :-

/************ Protect against filter_system_macros ************/
...
/*************************************************************/
  ...
  Xs ? "_complete_names"(Names, Tail),
  we(Tail) :
    Tail = [
/* pre-user_macros */	change_context, filter_user_macros, help,
/* pre-system-macros */	filter_system_macros, 
/* computations */	filter_computations,
/* status */		computation, service,
/* execute */		events, extract, monitor, new_goal, /* resolvent */
				reset, request, state, status, start
	   | Tail'?] |
	system_macros # expand("_complete_names"(Names, Tail'), Ys, Ys'),
	system_macros;

  Xs ? "_all_functors"(Names, Tail),
  we(Tail) :
    Tail = [
/* pre-user_macros */	change_context, filter_user_macros, help,
/* pre-system-macros */	filter_system_macros, 
...
/* execute */		events, extract, monitor, new_goal, /* resolvent */
	   ...
	   | Tail'?]
      |
	system_macros # expand("_all_functors"(Names, Tail'), Ys, Ys'),
	system_macros;
  ...
  tuple(X), arity(X) > 1,
  arg(2, X, Goal) :
    Ys ! service(Service, Service) |
	protect_system_macros(Service?, base, X, Goal, Ms, Ms', Ys', Ys''),
	system_macros;
  ...
  Xs =\= [_|_], Xs =\= [] :
    ...
    close_channel(DC),
    Ms = [] |
	broken_stream(filter_system_macros, Xs, Ys).


protect_system_macros(Service, Level, X, Goal, Ms, Ns, Ys, Zs) :-
  ...
  protect_sysmacs(Service, Level, X, Goal, Ms, Ns, Ys, Zs) :-

    arg(1, X, services), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, cd), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, input), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, close), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, load), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, edit), arity(X) < 4 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, compile), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, cdg), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, ndg), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, nccdg), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, wam), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, lint), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

    arg(1, X, type_check), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);
...
/*  Replace verify  */
    ...
    arg(1, X, debug), arity(X) < 6 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

/*  Replace verify  */
    arg(1, X, add), arity(X) < 5 |
	verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs);

/*
conjunction), arity(X) < Z |
  Conjunct = (A, B) |
...
*/
  ...
  verify_system_macro(Service, Level, X, Goal, Ms, Ns, Ys, Zs) :-
    ...
    Level =?= base :
      Goal = Service?,
      Ms = Ns |
% verify % screen#display(unify-we(Goal)-X),
	system_macros # expand(X, Ys, Zs);
    ...
    constant(Goal) :
      ...
      Service = _,
      Ms = Ns |
% verify % screen#display(constant(Goal)-X),
	system_macros # expand(X, Ys, Zs);
    ...
    unknown(Functor) :
      Level = _,
      Service = _,
      Ms = [defer(Functor, X) | Ns],
      Ys = Zs;

    Goal =?= #Goal',
    Level =?= base :
      Level' = other |
% verify % screen#display(iterate(Goal')-X),
	self;
...
% verify % screen#display(iterate(Goal')-X),
    ...
    otherwise :
      ...
      Service = _,
      Ms = Ns |
% verify % screen#display(otherwise(Goal)-X),
	system_macros # expand(X, Ys, Zs).
...
/*
** serve filter_computations(_,_) commands
** forward others for state handling and execution
*/
