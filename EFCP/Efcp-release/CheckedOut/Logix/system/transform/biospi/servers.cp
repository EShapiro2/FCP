...
-export([serve_empty_scope/8]).

-include(spi_constants).
-include(bio_constants).

/*
** serve_empty_scope/8+5
**
** Serve requests from first-level processes.  See serve_process_scope/9+6
**
** Input:
**
...
**
**      process(PiLHS, Status, NewChannelList, ProcessScope)
**
**          Analyze PiLHS (Left-hand-side) of process declaration;
**	    Return:
**               LHSS is a pair of process definintions:
**			{OuterLHS, InnerLHS}
**		 NewChannelList is a list of parameter descriptors:
**			Name(BaseRate) or Name(BaseRate, ComputeWeight) 
...
serve_empty_scope(In, Controls, Language,
		Exports, Ambients,
		Generated, Optimize, Errors)
    ...
    Controls =?= {Exported, Parameters, ParameterNames, WeightRate} |
	make_process_scope(PiLHS, WeightRate, Language,
				ProcessScope, [], ParameterNames,
	...
	/* sum_procedures. */
	call#sum_procedures(Language, Summed, Generated, Optimize, [], Errors).

  make_prefix_call(Export, Prefix) :-

    Export =?= true |
      Prefix = public_channels(_PublicPairList, VAR_SCHEDULER);
...
**      new_scope_id(Id)
**
**      process(PiLHS, LHSS, NewChannelList, ProcessScope)
**
**          Analyze PiLHS (Left-hand-side) of process declaration;
**	    Return:
**               LHSS is a pair of process definintions:
...
**
** Local: see serve_empty_scope/8+5 above.
*/

GUARD_REQUEST => Request(Guard, Message, Index, Guards).
CAPABILITY_REQUEST => capability(GUARD_REQUEST).

serve_process_scope(In, ProcessDefinition, WeightRate, Language,
		    Notes, Out, NextOut, Errors, NextErrors) +
    ...
    ProcessDefinition =?= {Name, _Arity, ChannelNames, _LHSS, _CodeTuple} :
      Primes = _,
      Guards = {Guard?, VAR_MESSAGE = ChannelList?},
	...
	verify_multiplier(Name, Multiplier?, Multiplier', ChannelNames,
				Errors', Errors''?),
	utilities#verify_communication_channel(Language, Name, Channel,
	...
	make_process_scope(PiLHS, WeightRate, Language,
				ProcessScope, ParameterNames, ParameterNames,
				NewChannelList,	In'', In'?,
    ...
    /* Dont Pass error */
    In ? Ambient, Ambient = ambient(_Id),
    Language =\= biospi :
      In'' = [error(ambient_not_allowed_in(Language)) | In'] |
	self;
...
make_process_scope(PiLHS, WeightRate, Language,
		   ProcessScope, ParameterNames, PublicNames,
	...
	serve_process_scope(ProcessScope?, NewDefinition, WeightRate, Language,
			    Notes', Out', NextOut, Errors''''', NextErrors).
...
extract_channel_list(Channels, WeightRate, ChannelList, NewChannelList,
  ...
  validate_new_weighter(Weighter, NewWeighter, Errors, NextErrors) :-
	...
	validate_new_weighter_params,
	utils#list_to_tuple([Variable, VAR_NULL | Params], NewWeighter);
