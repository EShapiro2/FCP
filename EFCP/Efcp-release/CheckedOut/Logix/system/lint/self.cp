...
lint(Source) + (Options = query) :-
    ...
    Source =?= Path#Name, list(Path), string(Name) :
      Source' = context(Path, Name) |
	self;

    Source =?= Path#Name, list(Path), Name = Element#Name' :
      Source' = [Element | Path] # Name' |
	self;

    Source =?= Prefix#_Suffix, constant(Prefix) |
	computation # Source # service_id(SId),
	SId? = [Name | Path],
	lint(context(Path?, Name?), Options);
